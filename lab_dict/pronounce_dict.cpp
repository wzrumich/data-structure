...
{
    string test1 = word1;
    std::transform(test1.begin(), test1.end(), test1.begin(), ::toupper);
    string test2 = word2;
    std::transform(test2.begin(), test2.end(), test2.begin(), ::toupper);
    auto lookup1 = dict.find(test1);
    auto lookup2 = dict.find(test2);
