...
SkipList::SkipList(int key, RGBAPixel value, int probability, int maxlevel) : SkipList(probability, maxlevel)
{
    insert(key, value);
}
...
SkipList::SkipList()
{
    ...
    this->head->value = RGBAPixel();

    this->tail->key = INT_MAX;
    this->tail->value = RGBAPixel();
 ...
 */
PNG SkipList::makeImage(int imgSize, RGBAPixel fg, RGBAPixel bg)
{
 ...
 */
PNG SkipList::toPNG(int maxHeight, RGBAPixel fg, RGBAPixel bg) const
{
    PNG ret(length + 2, maxHeight);     // the length doesn't include the sentinels

    toPNG(ret, maxHeight, head, 0, fg, bg);
 ...
 * @param bg The color of the background
 */
void SkipList::toPNG(PNG & png, int maxHeight, SkipNode * curr, size_t x, RGBAPixel fg, RGBAPixel bg) const
{
    if(curr == NULL)
        return;
    ...
    for(int y = 0; y < height; y++)
        *png(x, maxHeight - y - 1) = fg;
    for(int y = height; y < maxHeight; y++)
        *png(x, maxHeight - y - 1) = bg;

    // recurse down the nodes
    toPNG(png, maxHeight, curr->nodePointers[0].next, x + 1, fg, bg);
}
...
string SkipList::gdbGetNode(SkipNode * node) const
{
    ...
    string ret;
    char* s = new char[(9 + 20)](); // allocate enough memory for 9 chars + 2 10digit large numbers
    ...
    sprintf(s, "| %*s %d %*s |\n", (int)(width-to_string(node->key).size()/2), "", node->key, (int)(width-to_string(node->key).size()/2), "");
    ret += string(s);
